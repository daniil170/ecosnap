      ...
      <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-lg shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-extrabold text-slate-800 flex items-center gap-2">
            <Trophy className="text-orange-400" size={22} /> {t("achievements.Title")}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="rounded-3xl border border-slate-100 bg-slate-50 p-6 text-center">
          ...
          {/* ИСПРАВЛЕНИЕ: используем ключи перевода */}
          <h3 className="text-2xl font-black text-slate-900">
            {t(achievement.nameKey)}
          </h3>
          <p className="text-slate-500 mt-3">{t(achievement.descKey)}</p>
        ...
        <div className="mt-6 rounded-2xl border border-emerald-100 bg-emerald-50/50 p-4">
          ...
          <p className="mt-2 text-sm text-slate-700">
            {/* ИСПРАВЛЕНИЕ: передаем t в функцию, если она там нужна */}
