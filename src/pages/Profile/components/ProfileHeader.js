...
const ProfileHeader = ({ userData, user, onEdit }) => {
  ...
  return (
    <div
      className={`bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 shadow-sm border border-slate-100 dark:border-slate-700 mb-8 flex flex-col md:flex-row items-center gap-10 relative overflow-hidden transition-colors duration-300
      ${isGreenTheme ? "bg-gradient-to-r from-emerald-50 via-teal-50 to-lime-50 dark:from-emerald-900/20 dark:via-teal-900/20 dark:to-lime-900/20 border-emerald-200 dark:border-emerald-700" : ""}
      ${
    ...
    >
      ...
      <div className="relative flex-shrink-0">
        ...
        <div className="absolute -top-2 -right-2 bg-orange-500 text-white px-3.5 py-1.5 rounded-2xl shadow-xl border-2 border-white dark:border-slate-800 flex items-center gap-1.5">
      ...
      <div className="flex-1 text-center md:text-left">
        ...
        <div className="flex gap-2 mt-2 flex-wrap justify-center md:justify-start">
          {userData.badges?.includes("recycle") && (
            <span className="text-xs bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 px-3 py-1 rounded-full font-bold">
              {" "}
              {t("profile.badgeRecycle")}
            </span>
          )}
          {userData.badges?.includes("founder") && (
            <span className="text-xs bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 px-3 py-1 rounded-full font-bold">
              👑 Founder
            </span>
          )}
        </div>
        ...
        {userData.title && (
          <div className="mt-1 text-xs font-black uppercase text-emerald-500 dark:text-emerald-400">
            {userData.title}
          </div>
        )}
        ...
        {hasStickerPack && (
          <div className="mt-3 flex items-center gap-2 justify-center md:justify-start">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 px-2.5 py-1 text-xs font-bold">
              <Sparkles size={12} /> Eco Pack
            </span>
          </div>
        )}

        <div className="flex items-center gap-2 justify-center md:justify-start text-emerald-600 dark:text-emerald-400 font-bold text-sm mt-1">
          <span>@{userData.displayName || "eco_hero"}</span>
          <span className="w-1 h-1 bg-slate-300 dark:bg-slate-600 rounded-full" />
          <span className="flex items-center gap-1 text-slate-400 dark:text-slate-500">
            <MapPin size={14} /> {userData.city || t("profile.notSpecified")},{" "}
            {userData.country || t("profile.earth")}
          </span>
        </div>

        <div className="mt-6 max-w-md mx-auto md:mx-0">
          <div className="flex justify-between text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 mb-2 px-1 tracking-widest">
            <span>
              {t("profile.level")} {userData.level}
            </span>
            <span>{ecoScore % 100} / 100 ECO</span>
          </div>
          <div className="h-3.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden border border-slate-50 dark:border-slate-600 p-0.5">
            <div
              className="h-full bg-emerald-500 dark:bg-emerald-400 rounded-full transition-all duration-1000 shadow-[0_0_8px_rgba(16,185,129,0.4)]"
      ...
      <button
        onClick={onEdit}
        className={`p-4 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-2xl border transition-all active:scale-95 shadow-sm ${isGreenTheme ? "bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-700" : "bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700"}`}
      >
